...
- [x] **Fase B — Backend visibile** — completata 2026-05-20 (commit `7aecb57`). Segnale `current_scan_symbol` bridgeato in AppState (era orfano lato AppState); nuovo `_ScanChip` nella TopBar (StatusDot pulsante + simbolo + loop) dopo il bottone engine; reset idle dopo 5s, hide su engine stop. Separazione `current_scan_symbol` (engine) vs `current_symbol` (trader) rispettata.
...
- [x] **Fase D — UX ordini + auto-download** — completata 2026-05-20 (commit `a10c81b`). Diagnosi: auto-download già operativo (fetch async + cache disco). Vero gap = feedback visivo: `ChartPanel` ora mostra `StatusDot` 3 stati (loading/idle/error) + label contestuale durante il fetch; errori di rete/dati vuoti non più silenziosi. `data_panel` già rimosso in Fase E.
- [x] **Fase E — Pulizia + bug** — completata 2026-05-20 (commit `5b1fbed`). Indagine: watchlist NON duplicata (falso allarme), pattern pipeline integra (non rotta). Vero lavoro = codice morto: rimossi `data_panel.py` + `data_panel.ui` (orfani) e `analysis.py` (shim senza consumer); fix debito tecnico `PatternObserver` (property pubblica `observations` al posto di `._obs`).
